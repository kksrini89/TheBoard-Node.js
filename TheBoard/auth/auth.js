(function (auth) {
    ...
    auth.ensureAPIAuthenticated = function (req, res, next) {
        if (req.isAuthenticated()) {
            ...
            res.send(401, 'Not Authorzied!');
        }
    }
    auth.init = function (app) {
        
        //Setup passport authentication with express
        ...
        passport.deserializeUser(function (key, next) {
            data.getUser(key, function (err, user) {
                if (err) {
                    next(null, false, { message: "Error in retrieving user!" });
                } else {
                    next(null, user);
                }
            });
        ...
        app.get('/register', function (req, res) {
            res.render('register', { title: 'Registration For The Board' });
        });
